...
| `3. Получает предложенные доли` | Видит базовое распределение, может его скорректировать и отправить группе. | Меньше ручной математики и меньше трения в обсуждении. |
...
| `4. Возвращается в тот же Space` | Позже снова использует тот же Space на следующий повод и сам начинает приносить туда новые расходы. | Space живет дольше одного чека и становится привычным местом для группы. |
...
Переход к следующему разделу:
`Следующий вопрос — в каком порядке проверять эти гипотезы и какой минимальный продукт нужен для такой проверки.`
...
- Основная композиция: `1` таблица из `7` строк.
