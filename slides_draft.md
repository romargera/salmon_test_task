...
## Слайд 3.3. Решение и NE-петля для стартового сегмента

Основа: `llm_data/про основные сегменты.md` + `solution_structure_final.md` + `test_task.md`.

Цель:
Показать решение для стартового сегмента и объяснить, почему оно меняет поведение, а не добавляет еще один способ перевода.

Заголовок:
`Как это работает для друзей`

Контент:

Проблема:
`Сегодня общий расход проходит по схеме n-1-n: один платит, остальные возвращают через разные банки, кошельки и наличные. Учет долга и сам расчет разорваны.`

Текущее поведение:
`Чек -> таблица / Splitwise / GCash KKB -> реквизиты в чат -> переводы -> скриншоты -> ручная сверка.`

Предложенное решение:
`Группа под конкретный повод — поездку, ужин, подарок, событие. Внутри: расходы, split, напоминания и закрытие долга в один поток. История группы сохраняется между событиями.`

Короткая оговорка:
`Участнику группы не нужен счет Salmon в первый день: он может зайти по ссылке, увидеть свою долю и закрыть ее внешним банком или кошельком.`

Почему это ценно:
`Продукт убирает неловкость и ручную математику. Долг закрывается там же, где его увидели, без прыжков между таблицей, чатом и платежным приложением.`

NE-петля:

1. `Первый пользователь` создает группу под реальный расход.
2. `Второй пользователь` приходит не ради бонуса, а потому что без него нельзя завершить сценарий.
3. `Группа` получает общую историю расчетов и начинает использовать продукт повторно.
4. `Мост в следующий граф` возникает, когда тот же человек создает новую группу друзей в другом контексте.

Момент "без этого уже странно":
`Когда после нескольких событий вручную сводить долги в чате и скриншотах становится явно хуже, чем закрывать их внутри группы.`

Источники:

- [llm_data/про основные сегменты.md](/Users/roman/Documents/Pet Projects/Salmon test task/llm_data/про основные сегменты.md)
- [GCash KKB explainer](https://manilashaker.com/how-to-split-the-bill-using-gcash-kkb/)
- [Philippines payment rails: InstaPay / PESONet / GCash / Maya](https://www.transfi.com/blog/philippines-payment-rails-how-they-work---from-instapay-pesonet-to-gcash-maya-real-time-transfers)
...
- Основная композиция: `2 колонки` — слева `сегодня`, справа `решение`.
- Внизу отдельная горизонтальная полоса `NE-петля` из `4` шагов.
- Оговорку про отсутствие счета Salmon дать одной короткой строкой, не как отдельный блок.
